...
4. **Testing Strategy:** All testing must be performed inside running containers via Laravel Sail. Additionally, ParaTest has been installed, so parallel testing is available and should be utilized for optimum speed. To maintain compatibility with ParaTest, avoid using the RefreshDatabase trait, opting instead for the DatabaseTransactions trait. Tests must be written that cover at least 85% of lines in the main project files. When testing Filament components, extend the tests/Feature/Filament/Parent test files where appropriate, use the disabling traits in that same directory where appropriate, and add additional tests to achieve full coverage. Test files are never temporary, and should not be deleted. They should be designed to run in parallel, repeatedly without database reset.
